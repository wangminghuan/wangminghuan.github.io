...
对于真正的类语言来说，构造函数是属于类的。然而，在 JavaScript 中恰好相反——**实际上“类”是属于构造函数的!!!**，JS中是通过构造函数的原型链（类似Foo.prototype）来引用到对应的类的???????
...
- 多态
多态（Polymorphism）按字面的意思就是“多种状态”。也就是说：同一操作作用于不同的对象，可以有不同的解释，产生不同的执行结果。
多态的两种常用实现方式就是：覆盖和重载。JS中没有重载的概念，但是可以通过子类重新定义（覆盖）父类，这样就实现了“多态”。
在继承链的不同层次中一个方法名可以被多次定义。在子类中也可以相对引用它继承的父类，这种相对引用通常被称为`super`.但是由于JavaScript 中父类和子类的关系只存在于两者构造函数对应的`.prototype`对象中，因此它们的构造函数之间并不存在直接联系，从而无法简单地实现两者的相对引用（在 ES6 的类中可以通过 super 来“解决” 这个问题）。

由于JS在继承过程中并不会“自动”复制一份父类副本，因此，产生很多模拟类复制的解决方案（这些方案云里雾里不知道看了多少遍了~）
...
- [JavaScript有多态吗？有的话怎么实现？](https://www.zhihu.com/question/33596850)